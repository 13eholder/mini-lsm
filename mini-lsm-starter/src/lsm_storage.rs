...
pub(crate) struct LsmStorageInner {
    ...
    /// state.mem_table是无锁并发结构,只需要获取state的读锁即可,
    /// 因此可能会出现多个线程同时进行写操作,此时使用state_lock同步
...
impl LsmStorageInner {
    ...
    pub fn scan(
        ...
        let mtable_merge_iter = MergeIterator::create(mtable_iters);
        let mut l0_iters = Vec::with_capacity(state.sstables.len());
        for sst in state
            .l0_sstables
            .iter()
            .map(|sst_id| state.sstables[sst_id].clone())
